...
exports.parseMarinadeStake = parseMarinadeStake;
exports.fetchMarinadeAPY = fetchMarinadeAPY;
exports.fetchMsolExchangeRate = fetchMsolExchangeRate;
 ...
 */
function parseMarinadeStake(instruction, accountKeys, tokenBalanceChanges) {
    ...
    const solChange = Array.from(tokenBalanceChanges.entries()).find(([_, data]) => data.mint === 'SOL' // Native SOL has special handling
    );
    const msolChange = Array.from(tokenBalanceChanges.entries()).find(([_, data]) => data.mint === MSOL_MINT);
 ...
 */
async function fetchMarinadeAPY(connection) {
 ...
 */
async function fetchMsolExchangeRate(connection) {
