...
exports.assessMEVExposure = assessMEVExposure;
const transaction_1 = require("../types/transaction");
/**
 * Assess MEV exposure for a given transaction
 */
function assessMEVExposure(transaction) {
    const details = [];
    ...
    const hasTipAccount = transaction.instructions.some(ix => ix.accounts.some(acc => JITO_TIP_ACCOUNTS.includes(acc)));
    if (hasTipAccount) {
        hasJitoTip = true;
    ...
    transaction.instructions.forEach(ix => {
        if (ix.type === transaction_1.InstructionType.JUPITER_SWAP && ix.data) {
            const swap = ix.data;
    ...
    const computeBudgetIx = transaction.instructions.find(ix => ix.programId === 'ComputeBudget111111111111111111111111111111');
