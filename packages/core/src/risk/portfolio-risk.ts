...
import { RugPullRisk, detectRugPull } from './rug-detector';
...
export async function assessPortfolioRisk(
  ...
  const tokenRisks = new Map<string, RugPullRisk>();
  ...
  for (const balance of balances) {
    try {
      const risk = await detectRugPull(connection, new PublicKey(balance.mint));
  ...
  const sortedBalances = [...balances].sort((a, b) => 
    (b.valueUsd || 0) - (a.valueUsd || 0) || b.amount - a.amount
  );
